...
* (Default) No — use existing environments; no provisioning needed
* Yes — provision new infrastructure (triggers `infra-plan-template.md`)
* Custom — reference an existing environment the user specifies (ask for resource group, subscription, or config path)
...
* Local integration with containers — spin up dependencies via Docker Compose or similar
* Custom — user-defined testing strategy (ask for details)
* No — skip integration testing entirely

## Build Baseline

Should a build baseline be captured before migration to enable regression detection? Only applicable when integration testing is included; skip this question if integration testing is set to "No".

* (Default) Yes — capture a pre-migration baseline for comparison
* No — skip baseline capture
...
* (Default) Yes — include security/CVE remediation using the default security-plan-template
* Yes, with specific requirements — include security task with user-provided focus areas (ask for details, e.g., specific CVEs, compliance standards, or dependency audit scope)
