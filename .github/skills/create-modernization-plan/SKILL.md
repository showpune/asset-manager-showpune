...
5. **Generate plan and tasks**: Generate plan.md and tasks.json using the appropriate templates:
    ...
    **Integration Test Task Rules**: Add an integration test task when EITHER of these conditions is met:
     ...
     **Baseline Task Rules**: Add a setupBaseline task with type "setupBaseline" when all of these conditions are met:
    1. User request a integration test, the baseline environment is required to run the integration test, so we need to setup the baseline environment before the integration test.
    2. And the user explicitly requests a baseline setup (e.g., "setup baseline", "prepare baseline environment", "create baseline configuration")
